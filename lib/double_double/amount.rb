module DoubleDouble
  ...
  class Amount < ActiveRecord::Base
    ...
    scope :by_accountee,   ->(a) { where(accountee_id:  a.id, accountee_type:  a.class.base_class.to_s) }
    scope :by_context,     ->(c) { where(context_id:    c.id, context_type:    c.class.base_class.to_s) }
    scope :by_subcontext,  ->(s) { where(subcontext_id: s.id, subcontext_type: s.class.base_class.to_s) }
